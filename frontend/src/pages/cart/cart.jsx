...
export const Cart = () => {
  ...
  async function handleSubmit(){
    ...
    let productsName=Products.map(item=>{if(currentCart[item.id]!==0){
      return item.name
    }})
    let productsDescription=Products.map(item=>{if(currentCart[item.id]!==0){
  ...
  return (
    <>
    <div className="title_cart"><h1>Your Cart items</h1></div>
    {totalCost()>0?<div>
  <div className='cartProducts'>{Products.map((product)=>{return <CartItem data={product}/>})}</div>
  ...
  </div>:<div className='cart_empty'>Your cart is empty</div>
  } 
    </>
  )
}
